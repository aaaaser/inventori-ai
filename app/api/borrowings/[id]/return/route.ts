...
import { returnBorrowing } from '@/lib/school-inventory-service';
...
export async function POST(
  ...
  try {
    ...
    const itemConditions = body.itemConditions || [];

    const result = await returnBorrowing(numId, itemConditions, user);
    if (!result.success) {
      return NextResponse.json(
