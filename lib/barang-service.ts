...
import { Barang, BarangFormData, StatsData } from './types';
...
export async function getAllBarang(filters?: {
  kategori?: string;
  kondisi?: string;
  search?: string;
  sort?: string;
...
export async function getBarangList(filters?: {
  kategori?: string;
  kondisi?: string;
  search?: string;
  sortBy?: string;
...
export async function getBarangById(id: number): Promise<Barang | null> {
  const asset = await getSchoolAssetById(id);
  return asset ? mapAssetToLegacyBarang(asset) : null;
}

export async function createBarang(data: BarangFormData): Promise<ServiceResult<Barang>> {
  try {
    let jurId = 1;
...
export async function updateBarang(id: number, data: Partial<BarangFormData>): Promise<ServiceResult<Barang>> {
  try {
...
export async function deleteBarang(id: number): Promise<ServiceResult<boolean>> {
  try {
